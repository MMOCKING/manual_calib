...
void setRPY_YXZ(const tfScalar& roll, const tfScalar& pitch, const tfScalar& yaw, tf::Quaternion& quat)
	{
		// // ZXY
    ...
    // quat.setValue(sinYaw * cosPitch * cosRoll - cosYaw * sinPitch * sinRoll, //x
    //   cosYaw * sinPitch * cosRoll + sinYaw * cosPitch * sinRoll, //y
    ...
    quat.setValue(cosYaw * sinPitch * cosRoll + sinYaw * cosPitch * sinRoll, //x
            sinYaw * cosPitch * cosRoll - cosYaw * sinPitch * sinRoll, //y
            cosYaw * sinPitch * cosRoll - sinYaw * cosPitch * sinRoll, //z
            cosYaw * cosPitch * cosRoll + sinYaw * sinPitch * sinRoll); //w
...
void callback_updateExtrinsic(ilcc2::LocatorConfig& config, uint32_t level) {
  ...
  setRPY_YXZ(config.roll * d2r, config.pitch * d2r, config.yaw * d2r, quat);
...
void callback_LidarCamMsg(const sensor_msgs::PointCloud2ConstPtr& msg_pc,
                          const sensor_msgs::CompressedImageConstPtr& msg_img) {
  pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud(new pcl::PointCloud<pcl::PointXYZI>);
  pcl::fromROSMsg(*msg_pc, *input_cloud);
  image_now = cv_bridge::toCvCopy(msg_img, "bgr8")->image;
  // original: 1920*1080 crop to 640*480 for the center part
  image_now = image_now(cv::Rect(640, 300, 640, 480));
...
int main(int argc, char** argv) {
  ...
  camera_model = std::make_shared<ImageEst>(package_path + "/config/" + yaml_path);
  {
    ...
    setRPY_YXZ(-90 * deg2rad, 0, 180 * deg2rad, tf_quat);
  ...
  message_filters::Subscriber<sensor_msgs::CompressedImage> image_sub(nh, image_topic, 2);

  typedef sync_policies::ApproximateTime<sensor_msgs::PointCloud2, sensor_msgs::CompressedImage> MySyncPolicy;
